...
test('Create PUT API Request in Playwright', async ({ request }) => {
    ...
    const putAPIResponce = await request.put(`/booking/${bookingId}`, {
        headers: {
    ...
    expect(putAPIResponce.status()).toBe(200)
    expect(putAPIResponce.statusText()).toBe('OK')

    const putAPIJsonResponce = await putAPIResponce.json()
    console.log('PUT API Responce : ' + JSON.stringify(putAPIJsonResponce, null, 2))
})